...
def save_demand_supply_gap_plot(dir):
    ...
    fig.savefig(os.path.join(dir, 'DemandSupplyGap.eps'), format='eps', dpi=1000, bbox_inches = 'tight')
    plt.close()
...
def save_usage_vs_rebalance_plot(dir):
    ...
    plt.yscale('symlog')
    plt.xlabel('Hours in a Day', size=20)
    plt.ylabel('No. of Shareable Bikes (log)', size=20)
    plt.xticks(size=15, ticks=range(0, hours, 2))
    plt.yticks(size=15)
    plt.legend(fontsize=14)
    fig.savefig(os.path.join(dir, 'UsageVsRebalanceLog.eps'), format='eps', dpi=1000, bbox_inches = 'tight')
    plt.close()
    ...
    plt.yticks(size=15)
    plt.legend(fontsize=14)
    fig.savefig(os.path.join(dir, 'UsageVsRebalance.eps'), format='eps', dpi=1000, bbox_inches = 'tight')
    plt.close()
